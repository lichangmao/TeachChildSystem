...
void MyScene::SynShow(QList<records> points){
    pathitem=new QGraphicsPathItem;
    pathitem->setFlag(QGraphicsItem::ItemIsMovable);
    pathitem->setFlag(QGraphicsItem::ItemIsFocusable);
    pathitem->setAcceptHoverEvents(true);
    ...
    for(int i=0;i<points.size()-1;i++){
        pen.setColor(points.at(i).w);
        pathitem->setPen(pen);

        drawpath.moveTo(points.at(i).x,points.at(i).y);
        drawpath.lineTo(points.at(i+1).x,points.at(i+1).y);
    }
